    ...
    const fetchProjects = async () => {
      try {
        ...
        data?.forEach(p => {
          ...
          if (!['todo', 'inProgress', 'completed'].includes(statusKey)) statusKey = 'todo';
  ...
  const handleSave = async (formData) => {
    try {
      ...
      setProjects(prev => {
        ...
        // Remove from all columns (important if status changed)
        Object.keys(newState).forEach(k => {
          newState[k] = newState[k].filter(p => p.id !== savedProject.id);
        });

        // Map status to correct column key
        const column =
        ...
        newState[column] = [savedProject, ...newState[column]]; // add at top

        return newState; // <-- immediate UI update
      });
  ...
  const handleDone = async (projectId) => {
    try {
      ...
      setProjects(prev => {
        ...
        Object.keys(newState).forEach(k => {
          newState[k] = newState[k].filter(p => p.id !== projectId);
        });
        return newState;
      });
